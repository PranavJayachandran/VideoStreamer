...
const ChannelName = ({
  ...
  const [numberofMembers, setNumberOfMemebers] = useState<number>(0);
  const getData = async () => {
  ...
  const handleChannel = async () => {
    if (isMember == true) {
      let result = await leaveChannel(user_id);
      if (result.success) {
        setNumberOfMemebers(numberofMembers - 1);
        setIsMember(!isMember);
      }
    } else {
      let result = await joinChannel(user_id);
      if (result.success) {
        setNumberOfMemebers(numberofMembers + 1);
        setIsMember(!isMember);
      }
  ...
  return (
    <div className="px-10 flex gap-2 items-center py-2">
      ...
      <button
        className="ml-20 bg-red-600 px-2 py-2  text-sm  rounded"
